...
object ExamineAndTrain {
  ...
  def main(args: Array[String]) {
    ...
    println("------Sample Lang, Name, text---")
    sqlContext.sql("SELECT user.lang, id, user.name, user.location, text, geoLocation.latitude, geoLocation.longitude, place.name, place.country FROM tweetTable WHERE user.lang LIKE 'en'").collect().foreach(println)
    
    println("------Store info to the file text---")
    val twitterData = sqlContext.sql("SELECT user.lang, id, geoLocation.latitude, geoLocation.longitude, place.name, place.country, text FROM tweetTable WHERE geoLocation.latitude IS NOT NULL AND geoLocation.longitude IS NOT NULL")
    twitterData.saveAsTextFile("/tmp/tweetData")
