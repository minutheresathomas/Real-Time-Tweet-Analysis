...
val source = CSVFile("tagList0.csv") ~> IDColumn(1);

val tokenizer = {
  SimpleEnglishTokenizer() ~>            // tokenize on space and punctuation
  CaseFolder() ~>                        // lowercase everything
  WordsAndNumbersOnlyFilter() ~>         // ignore non-words and non-numbers
  MinimumLengthFilter(2)                 // take terms with >=3 characters StopWordFilter("en") 
}
...
val text = {
  ...
  TermMinimumDocumentCountFilter(3)      // filter terms in <4 docs TermDynamicStopListFilter(10)          // filter out 30 most common terms       DocumentMinimumLengthFilter(2)// take only docs with >=5 terms
}
...
val params = LDAModelParams(numTopics = 3, dataset = dataset,
